...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Alert, ActivityIndicator, Platform, Image, StatusBar } from 'react-native';
...
const handleConfirm = async () => {
  ...
  setSubmitting(true);
  try {
    // 👇 CHANGED KEYS TO MATCH BACKEND VALIDATOR
    const payload = {
      carId: car._id,
      startDate: startDate.toISOString(), // Changed from startDateTime
      endDate: endDate.toISOString()      // Changed from endDateTime
    };

    console.log("Sending Booking Payload:", payload); // Debugging line
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.section}>
            ...
            <View style={styles.dateCard}>
                    ...
                    <View style={{flex: 1}}>
                        ...
                        <View style={styles.pickerRow}>
                            ...
                            <TouchableOpacity onPress={() => openPicker('start', 'time')}>
                                <Text style={styles.pickerText}>{startDate.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
                ...
                <View style={styles.dateRow}>
                    <View style={[styles.dateIconBox, {backgroundColor: COLORS.navy[700]}]}>
                    ...
                    <View style={{flex: 1}}>
                        ...
                        <View style={styles.pickerRow}>
                            <TouchableOpacity onPress={() => openPicker('end', 'date')}>
                                <Text style={styles.pickerText}>{endDate.toLocaleDateString()}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => openPicker('end', 'time')}>
                                <Text style={styles.pickerText}>{endDate.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
        ...
        {/* Hidden Pickers */}
        {showStartPicker && <DateTimePicker value={startDate} mode={pickerMode} is24Hour={false} onChange={onChangeStart} />}
        {showEndPicker && <DateTimePicker value={endDate} mode={pickerMode} is24Hour={false} onChange={onChangeEnd} />}
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.btn} onPress={handleConfirm} disabled={submitting}>
            <LinearGradient colors={[COLORS.gold[500], '#FBBF24']} style={styles.gradientBtn}>
                {submitting ? <ActivityIndicator color={COLORS.navy[900]} /> : (
...
const styles = StyleSheet.create({
  ...
  card: { flexDirection: 'row', backgroundColor: COLORS.navy[800], borderRadius: 16, padding: 12, marginBottom: 24, borderWidth: 1, borderColor: COLORS.navy[700] },
  thumb: { width: 80, height: 80, borderRadius: 12, marginRight: 15, backgroundColor: COLORS.navy[700] },
  carName: { fontSize: 16, fontWeight: '700', color: COLORS.white, marginBottom: 4 },
  ...
  sectionLabel: { fontSize: 12, fontWeight: '700', color: COLORS.gray[400], marginBottom: 12, letterSpacing: 1 },
  
  dateCard: { backgroundColor: COLORS.navy[800], borderRadius: 16, padding: 16, borderWidth: 1, borderColor: COLORS.navy[700] },
  dateRow: { flexDirection: 'row', alignItems: 'center' },
  dateIconBox: { width: 40, height: 40, borderRadius: 10, backgroundColor: COLORS.navy[700], justifyContent: 'center', alignItems: 'center', marginRight: 15 },
  dateLabel: { fontSize: 12, color: COLORS.gray[400], marginBottom: 4 },
  pickerRow: { flexDirection: 'row', gap: 15 },
  pickerText: { fontSize: 15, fontWeight: '600', color: COLORS.white, textDecorationLine: 'underline' },
  dateDivider: { height: 1, backgroundColor: COLORS.navy[700], marginVertical: 16, marginLeft: 55 },

  breakdownCard: { backgroundColor: COLORS.navy[800], borderRadius: 16, padding: 20, borderWidth: 1, borderColor: COLORS.navy[700] },
  ...
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: COLORS.navy[900], padding: 20, borderTopWidth: 1, borderTopColor: COLORS.navy[700] },
