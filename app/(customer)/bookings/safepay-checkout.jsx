...
export default function SafepayCheckoutScreen() {
  ...
  const handleSuccess = () => {
    ...
    showAlert({
      ...
      message: `Your payment of PKR ${amount} has been processed.\n\nYour booking is now pending host confirmation.`,
      type: "success",
      buttons: [
        {
          text: "View My Trips",
  ...
  const isSafepayCheckoutUrl = (url) => {
    return (
      url.includes("sandbox.api.getsafepay.com") ||
  ...
  const isSuccessUrl = (url) => {
    ...
    return (
      url.includes("swiftride://payment/success") ||
  ...
  const isCancelUrl = (url) => {
    if (isSafepayCheckoutUrl(url)) return false; // Don't trigger on checkout page
    return (
      url.includes("swiftride://payment/cancel") ||
